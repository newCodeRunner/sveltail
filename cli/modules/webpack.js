...
module.exports = (env) => {
  ...
  if (platform === 'Electron') {
    ...
    electronConfig = {
      devServer: PROD ? undefined : {
        writeToDisk: true,
        open: false,
        contentBase: resolve(currDirectory, '.sveltail', 'Electron'),
        stats: {
          assets: true,
          children: false,
          chunks: false,
      ...
      output: {
        path: PROD ? resolve(currDirectory, 'dist', platform) : resolve(currDirectory, '.sveltail', 'Electron'),
        filename: PROD ? 'js/[name].js' : undefined,
        chunkFilename: PROD ? 'js/[id].js' : undefined,
        pathinfo: !PROD,
      },
  ...
  return electronConfig ? [config, electronConfig] : config;
};
