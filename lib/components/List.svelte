<script>
  ...
  const props = {
    ...
    items: helpers.isArray($$props.items)
      ? $$props.items.map((i, index) => {
          return { 
            id: index,
            title: helpers.isString(i.title) ? i.title : null,
            description: helpers.isString(i.description) ? i.description : null,
            icon: helpers.getIcon(i.icon),
            onClick() {
              if (helpers.isFunction(i.onClick)) i.onClick();
              dispatch('itemClicked');
            },
            cursor: helpers.isFunction(i.onClick),
          };
        }
      )
      : [],
...
<div class="bg-white text-black dark:bg-black dark:text-white {props.class}">
