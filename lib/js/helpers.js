...
const isNumber = (value) => typeof value === 'number' && value !== Infinity;
...
const isAllowedColor = (value) => [...Object.keys(colors), 'transparent'].findIndex((i) => i.split('-')[0] === value) > -1;
const isAllowedSize = (value) => ['xs', 'sm', 'md', 'lg', 'xl', 'auto'].findIndex((i) => i === value) > -1;
const isAllowedBreakpoint = (value) => Object.keys(screens).findIndex((i) => i === value) > -1;
...
export default () => ({
  ...
  getIcon(icn) {
    let icon = icn;
    if (isUndefined(icn) || isNull(icn) || !isString(icn)) icon = null;
