/* eslint-disable no-console */
const { resolve } = require('path');
const { readFileSync } = require('fs');
const sharp = require('sharp');

const { productIcons } = require('../../../package.json');

const logoPath = resolve(__dirname, '../../../', productIcons.logo);
const splashPath = resolve(__dirname, '../../../', productIcons['splash-logo']);
const logoType = productIcons.logo.split('.').pop();

if (logoPath !== null && logoType !== null) {
  const buffer = readFileSync(logoPath);

  const PWA = [16, 32, 72, 96, 120, 128, 144, 152, 180, 192, 384, 512];
  PWA.forEach((size) => {
    const outputPath = resolve(__dirname, '../../../', 'public/PWA/icons', `logo-${size}x${size}.png`);
    if (logoType === 'png') {
      sharp(buffer)
        .resize(size, size)
        .toFile(outputPath, (err) => {
          if (!err) console.log(`Saved image at ${outputPath}`);
        });
    } else {
      sharp(buffer)
        .png()
        .resize(size, size)
        .toFile(outputPath, (err) => {
          if (!err) console.log(`Saved image at ${outputPath}`);
        });
    }
  });

  const Web = [16, 32, 96, 128];
  Web.forEach((size) => {
    const outputPath = resolve(__dirname, '../../../', 'public/Web/icons', `logo-${size}x${size}.png`);
    if (logoType === 'png') {
      sharp(buffer)
        .resize(size, size)
        .toFile(outputPath, (err) => {
          if (!err) console.log(`Saved image at ${outputPath}`);
        });
    } else {
      sharp(buffer)
        .png()
        .resize(size, size)
        .toFile(outputPath, (err) => {
          if (!err) console.log(`Saved image at ${outputPath}`);
        });
    }
  });

  const Android = {
    'drawable-hdpi': {
      icon: '72×72',
      logo: '288×288',
      background: '576×768',
    },
    'drawable-ldpi': {
      icon: '36×36',
      logo: '144×144',
      background: '288×384',
    },
    'drawable-mdpi': {
      icon: '48×48',
      logo: '192×192',
      background: '384×512',
    },
    'drawable-xhdpi': {
      icon: '96×96',
      logo: '384×384',
      background: '768×1024',
    },
    'drawable-xxhdpi': {
      icon: '144×144',
      logo: '576×576',
      background: '1152×1536',
    },
    'drawable-xxxhdpi': {
      icon: '345×345',
      logo: '768×768',
      background: '1536×2048',
    },
  };

  Object.keys(Android).forEach((pathKey) => {
    Object.keys(Android[pathKey]).forEach((nameKey) => {
      const size = Android[pathKey][nameKey].split('×').map((i) => Number(i));
      const outputPath = resolve(__dirname, '../../../', 'src-nativescript/App_Resources/Android/src/main/res', pathKey, `${nameKey}.png`);

      if (nameKey === 'icon') {
        if (logoType === 'png') {
          sharp(buffer)
            .resize(size[0], size[0])
            .toFile(outputPath, (err) => {
              if (!err) console.log(`Saved image at ${outputPath}`);
            });
        } else {
          sharp(buffer)
            .png()
            .resize(size, size)
            .toFile(outputPath, (err) => {
              if (!err) console.log(`Saved image at ${outputPath}`);
            });
        }
      } else if (nameKey === 'background') {
        sharp({
          create: {
            width: size[0],
            height: size[1],
            channels: 4,
            // eslint-disable-next-line object-curly-newline
            background: productIcons['splash-background'],
          },
        })
          .png()
          .toFile(outputPath, (err) => {
            if (!err) console.log(`Saved image at ${outputPath}`);
          });
      }
    });
  });

  const iOS = {
    'icon-20.png': 20,
    'icon-20@2x.png': 40,
    'icon-20@3x.png': 60,
    'icon-29.png': 29,
    'icon-29@2x.png': 58,
    'icon-29@3x.png': 87,
    'icon-40.png': 40,
    'icon-40@2x.png': 80,
    'icon-40@3x.png': 120,
    'icon-60@2x.png': 120,
    'icon-60@3x.png': 180,
    'icon-76.png': 76,
    'icon-76@2x.png': 152,
    'icon-83.5@2x.png': 167,
    'icon-1024.png': 1024,
    'LaunchScreen-AspectFill.png': '768×1024',
    'LaunchScreen-AspectFill@2x.png': '1536×2048',
    'LaunchScreen-AspectFill@3x.png': '2304×3072',
  };

  Object.keys(iOS).forEach((nameKey) => {
    if (nameKey.includes('icon')) {
      const size = iOS[nameKey];
      const outputPath = resolve(
        __dirname, '../../../', 'src-nativescript/App_Resources/iOS/Assets.xcassets/', 'AppIcon.appiconset/', nameKey,
      );
      if (logoType === 'png') {
        sharp(buffer)
          .resize(size, size)
          .toFile(outputPath, (err) => {
            if (!err) console.log(`Saved image at ${outputPath}`);
          });
      } else {
        sharp(buffer)
          .png()
          .resize(size, size)
          .toFile(outputPath, (err) => {
            if (!err) console.log(`Saved image at ${outputPath}`);
          });
      }
    } else if (nameKey.includes('LaunchScreen')) {
      const bgSize = iOS[nameKey].split('×').map((i) => Number(i));
      const outputPath = resolve(
        __dirname, '../../../', 'src-nativescript/App_Resources/iOS/Assets.xcassets/', 'LaunchScreen.AspectFill.imageset/', nameKey,
      );
      sharp({
        create: {
          width: bgSize[0],
          height: bgSize[1],
          channels: 4,
          // eslint-disable-next-line object-curly-newline
          background: productIcons['splash-background'],
        },
      })
        .png()
        .toFile(outputPath, (err) => {
          if (!err) console.log(`Saved image at ${outputPath}`);
        });
    }
  });

  const Cordova = {
    icons: {
      'res/android': {
        'hdpi.png': 72,
        'ldpi.png': 36,
        'mdpi.png': 48,
        'xhdpi.png': 96,
        'xxhdpi.png': 144,
        'xxxhdpi.png': 192,
      },
      'res/ios': {
        'icon-1024.png': 1024,
        'icon-20.png': 20,
        'icon-20@2x.png': 40,
        'icon-20@3x.png': 60,
        'icon-24@2x.png': 48,
        'icon-27.5@2x.png': 55,
        'icon-29.png': 29,
        'icon-29@2x.png': 58,
        'icon-29@3x.png': 87,
        'icon-40.png': 40,
        'icon-40@2x.png': 80,
        'icon-44@2x.png': 88,
        'icon-50.png': 50,
        'icon-50@2x.png': 100,
        'icon-60@2x.png': 120,
        'icon-60@3x.png': 180,
        'icon-72.png': 72,
        'icon-72@2x.png': 144,
        'icon-76.png': 76,
        'icon-76@2x.png': 152,
        'icon-83.5@2x.png': 167,
        'icon-86@2x.png': 172,
        'icon-98@2x.png': 196,
        'icon.png': 57,
        'icon@2x.png': 114,
      },
    },
    splash: {
      'res/screen/android': {
        'splash-land-hdpi.png': '800×480',
        'splash-land-ldpi.png': '320×200',
        'splash-land-mdpi.png': '480×320',
        'splash-land-xhdpi.png': '1280×720',
        'splash-land-xxhdpi.png': '1600×960',
        'splash-land-xxxhdpi.png': '1920×1280',
        'splash-port-hdpi.png': '480×800',
        'splash-port-ldpi.png': '200×320',
        'splash-port-mdpi.png': '320×480',
        'splash-port-xhdpi.png': '720×1280',
        'splash-port-xxhdpi.png': '960×1600',
        'splash-port-xxxhdpi.png': '1280×1920',
      },
      'res/screen/ios': {
        'Default@2x~ipad~anyany.png': '2732×2732',
        'Default@2x~ipad~comany.png': '1278×2732',
        'Default@2x~iphone~anyany.png': '1334×1334',
        'Default@2x~iphone~comany.png': '750×1334',
        'Default@2x~iphone~comcom.png': '1334×750',
        'Default@3x~iphone~anyany.png': '2208×2208',
        'Default@3x~iphone~anycom.png': '2208×1242',
        'Default@3x~iphone~comany.png': '1242×2208',
      },
    },
    browser: [16, 32, 96, 128],
  };

  Cordova.browser.forEach((size) => {
    const outputPath = resolve(__dirname, '../../../', 'public/Cordova/icons', `logo-${size}x${size}.png`);
    if (logoType === 'png') {
      sharp(buffer)
        .resize(size, size)
        .toFile(outputPath, (err) => {
          if (!err) console.log(`Saved image at ${outputPath}`);
        });
    } else {
      sharp(buffer)
        .png()
        .resize(size, size)
        .toFile(outputPath, (err) => {
          if (!err) console.log(`Saved image at ${outputPath}`);
        });
    }
  });

  Object.keys(Cordova.icons).forEach((pathKey) => {
    Object.keys(Cordova.icons[pathKey]).forEach((nameKey) => {
      const size = Cordova.icons[pathKey][nameKey];
      const outputPath = resolve(__dirname, '../../../', 'src-cordova', pathKey, nameKey);

      if (logoType === 'png') {
        sharp(buffer)
          .resize(size, size)
          .toFile(outputPath, (err) => {
            if (!err) console.log(`Saved image at ${outputPath}`);
          });
      } else {
        sharp(buffer)
          .png()
          .resize(size, size)
          .toFile(outputPath, (err) => {
            if (!err) console.log(`Saved image at ${outputPath}`);
          });
      }
    });
  });

  if (splashPath !== null) {
    const bufferSplash = readFileSync(splashPath);

    Object.keys(Android).forEach((pathKey) => {
      Object.keys(Android[pathKey]).forEach((nameKey) => {
        if (nameKey === 'logo') {
          const size = Android[pathKey][nameKey].split('×').map((i) => Number(i));
          const outputPath = resolve(
            __dirname, '../../../', 'src-nativescript/App_Resources/Android/src/main/res', pathKey, `${nameKey}.png`,
          );

          sharp(bufferSplash)
            .resize(size[0], size[0])
            .toFile(outputPath, (err) => {
              if (!err) console.log(`Saved image at ${outputPath}`);
            });
        }
      });
    });

    const splashiOS = {
      'LaunchScreen-Center.png': 384,
      'LaunchScreen-Center@2x.png': 768,
      'LaunchScreen-Center@3x.png': 1152,
    };
    Object.keys(splashiOS).forEach((nameKey) => {
      const size = splashiOS[nameKey];
      const outputPath = resolve(
        __dirname, '../../../', 'src-nativescript/App_Resources/iOS/Assets.xcassets', 'LaunchScreen.Center.imageset', nameKey,
      );
      sharp(bufferSplash)
        .resize(size, size)
        .toFile(outputPath, (err) => {
          if (!err) console.log(`Saved image at ${outputPath}`);
        });
    });

    Object.keys(Cordova.splash).forEach((pathKey) => {
      Object.keys(Cordova.splash[pathKey]).forEach((nameKey) => {
        const bgSize = Cordova.splash[pathKey][nameKey].split('×').map((i) => Number(i));
        const splashSize = parseInt(Math.min(bgSize[0], bgSize[1]) * 0.6, 10);
        const outputPath = resolve(__dirname, '../../../', 'src-cordova', pathKey, nameKey);

        sharp(bufferSplash)
          .resize(splashSize, splashSize)
          .toBuffer()
          .then((data) => {
            sharp({
              create: {
                width: bgSize[0],
                height: bgSize[1],
                channels: 4,
                // eslint-disable-next-line object-curly-newline
                background: productIcons['splash-background'],
              },
            })
              .composite([{ input: data }])
              .png()
              .toFile(outputPath, (err) => {
                if (!err) console.log(`Saved image at ${outputPath}`);
              });
          });
      });
    });
  }
} else {
  console.log('Please define a logo path with --logo argument. Eg. --logo=./logo.png');
}
